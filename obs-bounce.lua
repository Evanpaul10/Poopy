...
-- Hitbox offsets to compensate for transparent padding
--- pixels to shrink hitbox from the left edge
local offset_left = 0
--- pixels to shrink hitbox from the right edge
local offset_right = 0
--- pixels to shrink hitbox from the top edge
local offset_top = 0
--- pixels to shrink hitbox from the bottom edge
local offset_bottom = 0
...
function script_properties()
   ...
   -- Hitbox offset controls
   obs.obs_properties_add_int_slider(props, 'offset_left', 'Hitbox Offset - Left:', 0, 500, 1)
   obs.obs_properties_add_int_slider(props, 'offset_right', 'Hitbox Offset - Right:', 0, 500, 1)
   obs.obs_properties_add_int_slider(props, 'offset_top', 'Hitbox Offset - Top:', 0, 500, 1)
   obs.obs_properties_add_int_slider(props, 'offset_bottom', 'Hitbox Offset - Bottom:', 0, 500, 1)
...
function script_defaults(settings)
   ...
   obs.obs_data_set_default_int(settings, 'offset_left', offset_left)
   obs.obs_data_set_default_int(settings, 'offset_right', offset_right)
   obs.obs_data_set_default_int(settings, 'offset_top', offset_top)
   obs.obs_data_set_default_int(settings, 'offset_bottom', offset_bottom)
end
...
function script_update(settings)
   ...
   -- Read hitbox offset settings
   offset_left = obs.obs_data_get_int(settings, 'offset_left')
   offset_right = obs.obs_data_get_int(settings, 'offset_right')
   offset_top = obs.obs_data_get_int(settings, 'offset_top')
   offset_bottom = obs.obs_data_get_int(settings, 'offset_bottom')
...
function move_scene_item(scene_item)
   ...
   -- Apply hitbox offsets to collision detection
   local hitbox_left = pos.x + offset_left
   local hitbox_right = pos.x + width - offset_right
   local hitbox_top = pos.y + offset_top
   local hitbox_bottom = pos.y + height - offset_bottom

   if moving_right and hitbox_right < scene_width then
      next_pos.x = math.min(pos.x + speed, scene_width - width + offset_right)
   else
      moving_right = false
      next_pos.x = math.max(pos.x - speed, -offset_left)
      if next_pos.x == -offset_left then moving_right = true end
   end

   if moving_down and hitbox_bottom < scene_height then
      next_pos.y = math.min(pos.y + speed, scene_height - height + offset_bottom)
   else
      moving_down = false
      next_pos.y = math.max(pos.y - speed, -offset_top)
      if next_pos.y == -offset_top then moving_down = true end
   end
...
function throw_scene_item(scene_item)
   ...
   local was_bottomed = hitbox_bottom >= scene_height

   next_pos.x = pos.x + velocity_x
   next_pos.y = pos.y + velocity_y

   -- Recalculate hitbox after movement
   local next_hitbox_bottom = next_pos.y + height - offset_bottom
   local next_hitbox_left = next_pos.x + offset_left
   local next_hitbox_right = next_pos.x + width - offset_right

   -- bounce off the bottom
   if next_hitbox_bottom >= scene_height then
      next_pos.y = scene_height - height + offset_bottom
   ...
   if next_hitbox_right >= scene_width or next_hitbox_left <= 0 then
      if next_hitbox_left <= 0 then
         next_pos.x = -offset_left
      else
         next_pos.x = scene_width - width + offset_right
      end
   ...
   -- Check if on ground using hitbox
   if next_pos.y + height - offset_bottom >= scene_height then
