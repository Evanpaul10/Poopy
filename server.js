...
function render(id){
  ...
  let url=\`${VDO}/?view=\${encodeURIComponent(id)}&cleanoutput=1&stats=0&autostart=1&cover&relay\`;
...
function render(){
  ...
  active.forEach(([slotNum,slot])=>{
    ...
    if(!streamId){
      ...
      let url=\`${VDO}/?view=\${encodeURIComponent(streamId)}&cleanoutput=1&stats=0&autostart=1&cover&relay\`;
      if(!isOBS())url+="&muted=1";
